...
{
    sqlite3_stmt* stmt;
    sqlite3_prepare_v2(db, wxString::Format("SELECT GROUP_CONCAT(name, ';') FROM %s WHERE id IN (SELECT metadata_id FROM metadata_mapping WHERE game_id==%d)", type, gameId).mb_str(), -1, &stmt, NULL);
    sqlite3_step(stmt);
